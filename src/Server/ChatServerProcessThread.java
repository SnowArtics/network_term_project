...
public class ChatServerProcessThread extends Thread {
	...
	public ChatServerProcessThread(Socket socket, List<PrintWriter> listWriters, List<ClientInfo> clientInfo) {
		this.socket = socket;
	...
	public void run() {
		try {
			buffereedReader = new BufferedReader(
					new InputStreamReader(socket.getInputStream(), StandardCharsets.UTF_8)); // 클라이언트에게 받고

			PrintWriter printWriter = new PrintWriter(
					new OutputStreamWriter(socket.getOutputStream(), StandardCharsets.UTF_8));// 클라이언트에게 보낸다
			...
			while (true) {
				...
				if ("join".equals(tokens[0])) {// join이면 doJoin 실행
					...
					if (functionTokens[0].equals("/invite")) {
						...
						System.out.println(nickname+"이 보낸 메세지입니다:"+ request);
						doMessage(tokens[1]);
					}
	...
	private void addClient(String nickname, Socket socket, PrintWriter writer) {// 클라이언트객체 리스트에 클라이언트들을 저장
		synchronized (clientInfo) {
			...
			for (int i = 0; i < clientInfo.size(); i++) {
				System.out.println(clientInfo.get(i).getNickName() + clientInfo.get(i).getSocket()
						+ clientInfo.get(i).getThisWriter() + clientInfo.get(i));
			}
		}
	}
	...
	/*
	 * private void invite(String invitee) { int count =0;
	 * if(thisClientInfo.getRoom()==null) { gameRoom =
	 * roomManager.createRoom(thisClientInfo);//현재 이 쓰레드를 사용하는 클라이언트의 room이 널이라면 그사람
	 * 기준으로 방을 하나 만들어줌 listWriters.remove(thisWriter);//다른방에 들어간 플레이어의
	 * thiswriter삭제해줌 삭제함으로써 이 사람을 제외한 사람들에게만 보낸다 //buffereedReader = null;//읽지도 않는다
	 * } for(int i=0;i<clientInfo.size();i++) {
	 * if(clientInfo.get(i).getNickName().equals(invitee)) { count = i;
	 * gameRoom.enterUser(clientInfo.get(i));//해당 방에 유저가 들어옴
	 * listWriters.remove(clientInfo.get(count).getThisWriter());////다른방에 들어간 플레이어의
	 * thiswriter삭제해줌 삭제함으로써 이 사람을 제외한 사람들에게만 보낸다 } } for(PrintWriter writer :
	 * listWriters) { writer.println("delete:"+thisClientInfo.getNickName());
	 * writer.flush();
	 * writer.println("delete:"+clientInfo.get(count).getNickName());
	 * writer.flush(); } gameRoom.run();
	 * 
	 * thisClientInfo.getThisWriter().println("invite:");
	 * thisClientInfo.getThisWriter().flush();
	 * clientInfo.get(count).getThisWriter().println("invite:");//해당 방에 들어간 유저한테
	 * invite신호를보냄 invite신호를 받은 ChatGUI는 새로운 파티 RoomGUI를 띄운다.
	 * clientInfo.get(count).getThisWriter().flush();
	 * 
	 * synchronized(clientInfo) { for(int i=0;i<clientInfo.size();i++) {
	 * if(clientInfo.get(i).equals(thisClientInfo)) {
	 * clientInfo.get(i).getThisWriter().println("invite:");
		...
		synchronized (listWriters) {
			for (PrintWriter writer : listWriters) {// 모든 printwirter에 있는 리스트에 브로드캐스팅한다.
				if (thisWriter.equals(writer)) {// 새롭게 들어온 사람한테는 현재 접속해 있는 모든 사람의 정보를 보낸다
					...
					writer.println("nickname:" + clientInfo.get(clientInfo.size() - 1).getNickName());// 그냥 새롭게 들어온사람만
																										// 추가해 준다.
					writer.flush();
				}
			}
